...
impl Database {
    ...
    pub fn get_event(&self, event_id: i32) -> Option<Event> {
        events
            .find(event_id)
    ...
    pub fn delete_event(&self, delete_id: i32) -> Result<usize, diesel::result::Error> {
        diesel::delete(events.filter(id.eq(delete_id))).execute(&mut self.pool.get().unwrap())
    }
